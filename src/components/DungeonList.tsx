...
export default function DungeonList({ onStartDungeon }: Props) {
  ...
  return (
    <div className="dungeon-list">
      <h3 style={{ textAlign: 'center', color: '#f0c040', marginBottom: 12 }}>🗺️ 取经副本</h3>
      ...
      {DUNGEONS.map(d => {
        ...
        return (
          ...
          >
            ...
            <div style={{ fontSize: 11, color: '#8b8b9e', margin: '4px 0' }}>
              {d.description}
            </div>

            <div style={{ fontSize: 11, color: '#8b8b9e' }}>
              🪙 {formatNumber(d.rewards.lingshi[0])}~{formatNumber(d.rewards.lingshi[1])} · 🍑×{d.rewards.pantao}
            ...
            <div style={{ marginTop: 6 }}>
              {canGo ? (
                <button className="small-btn accent" onClick={() => onStartDungeon(d.id)}>
                  ⚔️ {isCleared ? `挑战 (${attempts}/${d.dailyLimit})` : '首次挑战'}
                </button>
              ) : (
                <span style={{ fontSize: 11, color: '#f44336' }}>🔒 {lockReason}</span>
              )}
            </div>
          </div>
        );
