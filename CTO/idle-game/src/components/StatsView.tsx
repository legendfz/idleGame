...
export function StatsView() {
  ...
  const stats = [
    { icon: '🏔️', label: '当前境界', value: realm?.name ?? '凡人' },
    { icon: '📊', label: '等级', value: `Lv.${player.level}` },
    { icon: '⏱️', label: '总游戏时间', value: formatTime(totalPlayTime) },
    ...
    { icon: '⬆️', label: '突破次数', value: (player.totalBreakthroughs || 0).toString() },
    { icon: '🎁', label: '装备掉落', value: (player.totalEquipDrops || 0).toString() },
    { icon: '🎒', label: '背包装备', value: inventory.length.toString() },
    { icon: '🗺️', label: '最高关卡', value: `${highestChapter}-${highestStage}` },
    { icon: '⚔️', label: '攻击力', value: formatNumber(player.stats.attack) },
    { icon: '❤️', label: '生命值', value: formatNumber(player.stats.maxHp) },
      ...
      <div style={{
        ...
        {stats.map(s => (
          <div key={s.label} style={{
            background: 'var(--bg-card, #1e1e2e)', borderRadius: 10, padding: '12px 10px',
