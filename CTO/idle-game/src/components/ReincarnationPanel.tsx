...
export function ReincarnationPanel() {
  ...
  return (
    <div className="main-content fade-in" style={{ padding: 'var(--space-md)' }}>
      ...
      {/* Perks */}
      <h3 className="section-title" style={{ marginTop: 16 }}>✨ 永久加成 (道点商店)</h3>
      {REINC_PERKS.map(perk => {
        ...
        return (
          <div key={perk.id} className="card" style={{ marginBottom: 8, display: 'flex', alignItems: 'center', gap: 10 }}>
            <div style={{ fontSize: 24, minWidth: 36, textAlign: 'center' }}>{perk.icon}</div>
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 12, color: 'var(--text-dim)' }}>{perk.desc}</div>
            </div>
            <button
